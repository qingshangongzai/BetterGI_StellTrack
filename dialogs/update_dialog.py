...
"""检查更新对话框模块，负责从Gitee仓库获取最新版本信息，并提供版本比较和跳转功能。"""
...
from PyQt6.QtGui import QFont, QDesktopServices, QTextOption
from PyQt6.QtWidgets import QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QTextEdit, QProgressBar
...
from styles import UnifiedStyleHelper, get_global_font_manager, BaseFramelessDialog, ChineseMessageBox


GITEE_API_URL = "https://gitee.com/api/v5/repos/qingshangongzai/BetterGI_StellTrack/releases/latest"
GITEE_RELEASE_URL = "https://gitee.com/qingshangongzai/BetterGI_StellTrack/releases"


class UpdateCheckThread(QThread):
    """检查更新的后台线程

    在后台线程中检查Gitee仓库的最新版本信息，避免阻塞主线程。
    """
...
class UpdateDialog(BaseFramelessDialog):
    """检查更新对话框

    提供版本检查功能，显示当前版本和最新版本信息，并提供访问发行页面的功能。
    """
    ...
    def setup_ui(self):
        ...
        title_label.setStyleSheet(f"color: {style_helper.COLORS['primary']};")
    ...
    def open_release_page(self):
        """打开Gitee发行版页面"""
        try:
            result = webbrowser.open(GITEE_RELEASE_URL)

            if not result:
                success = QDesktopServices.openUrl(QUrl(GITEE_RELEASE_URL))

                if not success:
                    raise RuntimeError("两种方法都无法打开浏览器")
