...
- **虚拟环境优先原则**：所有开发和调试工作应优先在虚拟环境中进行，避免污染系统Python环境
- **虚拟环境创建**：
  - 使用Python内置的`venv`模块创建虚拟环境：`python -m venv venv`
  - 激活虚拟环境：`.\venv\Scripts\activate`（Windows）或`source venv/bin/activate`（Linux/Mac）
  - 在项目根目录创建`venv`文件夹，并将虚拟环境路径添加到`.gitignore`文件
- **依赖管理**：
  - 在虚拟环境中安装所有项目依赖：`pip install -r requirements.txt`
  - 新增依赖时，及时更新`requirements.txt`：`pip freeze > requirements.txt`
  - 定期检查并清理未使用的依赖包
- **调试环境配置**：
  - 在虚拟环境中配置调试工具和IDE插件
  - 确保IDE使用虚拟环境的Python解释器
  - 配置断点调试和变量监视功能
- **虚拟环境共享**：
  - 提供虚拟环境创建和激活的脚本
  - 在项目文档中说明虚拟环境的使用方法
  - 使用`requirements.txt`确保团队成员环境一致
...
|2.7 |2026-01-10 |重构样式模块，将 styles.py 拆分为 styles/ 目录结构，包含 fonts.py、themes.py、widgets.py、dialogs.py、**init**.py，保持向后兼容性，提高代码可维护性 |
