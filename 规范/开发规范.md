...
|UI模块 |界面组件和布局，包含主窗口和各种功能面板 |`main_window.py`、`panels.py`、`widgets.py` |
|菜单模块 |菜单栏创建和管理，处理菜单项的状态和交互 |`menu_manager.py` |
|对话框模块 |各类事件编辑对话框和调试对话框的实现 |`event_dialogs.py`、`batch_dialog.py`、`debug_dialog.py` |
...
|状态管理模块 |状态保存、加载、撤销和重做功能，处理事件数据的持久化 |`state_manager.py` |
