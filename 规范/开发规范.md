...
基于版权完善计划实施经验，以下实践可提高许可证信息维护的效率和准确性：
...
在添加新第三方库时，必须收集以下信息：
- 库名称和版本要求
- 许可证类型（如 GPL-3.0-only、MIT、Apache-2.0、BSD-3-Clause 等）
- 版权所有者（个人作者或组织）
- 项目地址（GitHub 仓库或官方网站）
- 许可证文本（完整文本或引用说明）
...
- 采用清晰的章节划分（【字体】、【核心框架】、【运行时依赖库】、【开发工具链】等）
...
| 许可证 | 兼容性 | 说明 |
|--------|--------|------|
| GPL-3.0-only | ✓ 一致 | 与项目主许可证一致 |
| Apache-2.0 | ✓ 兼容 | 与 GPLv3 兼容 |
| BSD-3-Clause | ✓ 兼容 | 与 GPLv3 兼容 |
| MIT | ✓ 兼容 | 与 GPLv3 兼容 |
| PSF License | ✓ 兼容 | 与 GPLv3 兼容 |
| GPL-2.0+ | ✓ 兼容 | 可升级到 GPLv3 |
| Modified BSD | ✓ 兼容 | 与 GPLv3 兼容 |
| SIL OFL 1.1 | ✓ 兼容 | 字体许可证，与 GPLv3 兼容 |

**不兼容的许可证（避免使用）**：
- GPL-2.0-only（与 GPLv3 不兼容，除非明确允许升级）
- 专有许可证（与 GPLv3 冲突）
...
2. 检查许可证兼容性
3. 更新 LICENSE 文本文件
4. 更新 LICENSE.html 文件
5. 更新关于窗口（`dialogs/about_window.py`）
6. 更新版权完善计划文档（如有）
7. 验证所有文件的显示效果
...
**需要声明的工具链**：
- 打包工具（如 auto-py-to-exe、PyInstaller）
- 压缩工具（如 UPX）
- 安装程序制作工具（如 Inno Setup）
- 构建工具（如 setuptools、build、wheel）
...
**字体许可证特殊要求**：
- 字体通常使用 SIL Open Font License 1.1
- 需要明确标注字体版权所有者
- 需要说明字体的用途（如 LOGO、标题、正文等）
- 字体文件应放在独立的 `fonts/` 目录中

**得意黑和思源宋体示例**：
```
字体名称：Smiley Sans (得意黑)
版权所有：© atelier-anchor
许可证：SIL Open Font License 1.1
项目地址：https://atelier-anchor.com/typefaces/smiley-sans/
用途：LOGO、标题显示

字体名称：Source Han Serif (思源宋体)
...
**问题1：如何获取第三方库的许可证信息？**
解决方案：
- 使用 `pip show <库名>` 查看基本信息
- 查看库的官方文档或 GitHub 仓库
- 查看库的安装目录中的 LICENSE 文件
- 访问开源许可证数据库（如 Open Source Initiative）
...
**问题3：如何处理许可证文本过长的情况？**
解决方案：
- 对于与主项目许可证相同的库（如 GPLv3），可以引用主许可证章节
- 对于常见的许可证（如 MIT），可以在一个章节中列出所有使用该许可证的库，然后提供一次完整文本
- 确保引用清晰，便于用户查找
...
解决：使用 `TitleBarThemeMixin` 混入类，该类会自动处理 Windows 10+ 系统的标题栏深色/浅色模式切换。所有继承 `StyledDialog` 或 `StyledFramelessMainWindow` 的窗口都会自动支持标题栏主题切换，无需手动实现。
...
|2.26 |2026-02-08 |完善开源许可证合规性规范(14.7)，添加许可证信息维护实践经验，包含第三方库信息收集规范、许可证文件格式规范、许可证兼容性检查、许可证信息更新流程、开发工具链许可证管理、字体许可证管理、验证清单、常见问题解决方案和版权完善计划模板 |
|2.27 |2026-06-06 |更新15.6节问题5，将已废弃的 `StyledMainWindow` 引用改为 `StyledFramelessMainWindow`，反映项目全面迁移至无边框窗口（PyQt6-Frameless-Window）的架构变更；清理旧 `StyledMainWindow` 类和相关冗余代码 |
