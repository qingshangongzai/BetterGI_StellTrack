...
BetterGI StellTrack（BetterGI 星轨）是一款专为 BetterGI（一款《原神》自动化辅助工具）打造的键鼠脚本生成与管理工具，致力于生成强大的键鼠自动化脚本。该工具基于 PyQt6 开发，提供了直观的可视化界面，允许用户创建复杂的自动化操作序列。其核心初衷是实现游戏内的延时摄影——通过自动控制图片的截取和保存，创作出如电影般壮丽的视觉诗篇，是一个为你手中的"提瓦特影机"赋予生命的工具。
...
- **多种事件类型支持**：支持按键按下/释放、鼠标移动、点击、滚轮等多种事件类型，满足延时摄影的复杂操作需求
- **批量编辑功能**：支持对多个事件进行批量修改，提高延时摄影脚本的编辑效率
- **精准时间逻辑管理**：灵活的时间逻辑设置，支持事件时间的自动计算和调整，确保延时摄影的流畅性
- **脚本生成与导出**：将事件序列生成为适用于BetterGI的脚本文件，专为延时摄影创作设计
- **事件时间分析**：提供事件时间分布和统计分析功能，帮助优化延时摄影的时间节奏
- **屏幕信息检测**：自动检测屏幕分辨率和缩放比例，确保延时摄影脚本的适配性
- **调试工具**：内置调试功能，方便延时摄影脚本的开发和问题排查

### 适用场景

- **游戏内延时摄影创作**：通过自动控自动控制图片的截取和保存，创作如电影般壮丽的《原神》场景延时摄影
- 重复性键鼠操作的自动化
- 游戏辅助工具开发与测试
- 键鼠事件序列的记录与回放
...
事件编辑是 BetterGI StellTrack 的核心功能，用户可以通过表格界面直观地管理键鼠事件，专为延时摄影场景设计。

**主要功能区域**：

- 事件表格：显示所有事件的详细信息，包括序号、事件名称、事件类型、键码、时间等，便于延时摄影脚本的精细化编辑
- 操作按钮：提供添加、编辑、删除、复制、粘贴等操作，支持延时摄影复杂序列的快速调整
- 搜索过滤：支持按事件类型和关键字搜索事件，方便在大型延时摄影脚本中定位特定事件

### 2. 事件添加与编辑

用户可以通过「添加事件」按钮打开事件编辑对话框，设置事件的各项属性，满足延时摄影的精确控制需求：

- 事件类型：选择按键按下/释放、鼠标移动、点击等类型，支持延时摄影的镜头控制和时间流逝模拟
- 按键/鼠标信息：设置具体的按键或鼠标操作，实现精确的镜头移动和视角调整
- 时间设置：设置事件的相对时间和延迟，精确控制延时摄影的节奏和流畅度

### 3. 批量编辑功能

批量编辑功能允许用户同时修改多个事件的属性，大幅提高延时摄影脚本的编辑效率：

- 增减偏移时间：对选中事件的时间进行统一增减，快速调整延时摄影的整体节奏
- 统一相对时间：将选中事件的相对时间设置为相同值，实现延时摄影中的重复动作序列
- 事件类型替换：将一种事件类型替换为另一种类型，便于延时摄影脚本的批量调整

### 4. 脚本生成与导出

完成事件编辑后，用户可以点击「生成脚本」按钮将事件序列转换为适用于BetterGI的脚本文件，专为延时摄影创作设计。生成的脚本可以保存到本地，并在需要时通过BetterGI运行，实现自动延时摄影效果。

### 5. 事件时间分析

事件时间分析功能提供了事件时间分布的可视化展示，帮助优化延时摄影的时间节奏：

- 时间分布直方图：显示事件时间的分布情况，便于调整延时摄影的节奏变化
- 统计信息：显示事件总数、平均时间、最大/最小时间等统计数据，辅助延时摄影的时间规划
- 时间序列图：展示事件时间的序列变化，直观呈现延时摄影的整体流程和节奏
