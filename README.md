...
- [x] 增加暗色主题，并提供切换菜单（暗色主题尝试过很多次，效果都不理想，但这次终于实现了这个目标）
- [x] 按钮和输入框高度不统一（已进行调整）
- [ ] **部分样式还没有统一**（AI写的屎山代码，改起来特别的困难）
- [ ] **设计语言中规范中还存在着一些未修正、未完善的部分的部分，没有改进**
- [ ] 但部分控件依旧存在直角边
- [ ] 菜单圆角存在锯齿（因为菜单的圆角是裁切得来的，所以会造成锯齿，但是如果不采用圆角裁切，四个圆角又不统一）
- [ ] 标题栏（窗口栏）修改为纯白色(创建自定义标题栏，取代系统标题栏）
- [ ] 事件可视化时间轴（之前尝试做过，但是出现了显示重叠等多种多样的问题）
- [ ] 大量事件生成时的进度条对话框的取消按钮响应慢（可能多多少少因为性能原因，也可能是因为检查点太少了，很有可能出现无响应的情况。）
- [ ] 事件列表与滚动条相连接的位置存在一条垂直线，无法去除
- [ ] 打包环境下复选框的样式变成了Qt原生样式（从3.13开始的新bug，AI一直没找到原因，修复了好几次，都没有修复好……其实之前为了实现复选框的“正常显示”也花了很多功夫，但是不知道AI哪根筋搭错了，在修改的过程中一统乱改，让这部分出现了问题）。
